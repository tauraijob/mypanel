...
export default defineEventHandler(async (event) => {
   ...
   doc.fontSize(10)
      .fillColor(slate500)
      .font('Helvetica')
      .text(settings?.invoiceFooter || 'Thank you for your business!', margin, pageHeight - 50, { width: pageWidth - (margin * 2), align: 'center' })

   doc.fontSize(8)
      .fillColor(slate400)
      .text(`Generated ${formatDate(new Date())}`, margin, pageHeight - 35, { width: pageWidth - (margin * 2), align: 'center' })

   // Powered by MyPanel signature
   doc.fontSize(8)
      .fillColor(primaryBlue)
      .font('Helvetica')
      .text('Powered by MyPanel', margin, pageHeight - 20, { width: pageWidth - (margin * 2) - 60, align: 'center', link: 'https://mypanel.wecode.co.zw/signup' })
   doc.fillColor(slate400)
      .font('Helvetica')
      .text(' - mypanel.wecode.co.zw', margin + (pageWidth - margin * 2) / 2 + 45, pageHeight - 20)
